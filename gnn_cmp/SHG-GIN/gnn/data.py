...
class AtomInitializer(object):
    ...
    def get_atom_fea(self, atom_type):
        atom_type_simplified = "".join(
            [c for c in atom_type if not (c.isdigit()) and (not c in ["+", "-", "."])]
        )
        if atom_type_simplified not in self.atom_types:
            print(atom_type_simplified)
        assert atom_type_simplified in self.atom_types
        return self._embedding[atom_type_simplified]
...
class CIFData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        atom_fea = np.vstack(
            [self.ari.get_atom_fea(str(crystal[i].specie)) for i in range(len(crystal))]
        )
        ...
        nbr_fea_idx = torch.LongTensor(
            [list(map(lambda x: x[2], nbrs[: self.max_num_nbr])) for nbrs in all_nbrs]
        )
        nbr_fea_d = torch.Tensor(
            [list(map(lambda x: x[1], nbrs[: self.max_num_nbr])) for nbrs in all_nbrs]
        )

        cart_coords = torch.Tensor(
            np.array([crystal[i].coords for i in range(len(crystal))])
        )
        ...
        angle_cosines = torch.matmul(dxyz, torch.swapaxes(dxyz, 1, 2)) / torch.matmul(
    ...
    def format_adj_matrix(self, adj_matrix):
        ...
        all_src_nodes = (
            torch.tensor([[x] * adj_matrix.shape[1] for x in src_list])
            .view(-1)
            .long()
            .unsqueeze(0)
        )
...
class CIFData_nogap(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        nbr_fea_idx = self.format_adj_matrix(torch.LongTensor(nbr_fea_idx))
        target = torch.Tensor([float(target)])
        return (atom_fea, nbr_fea, nbr_fea_idx), None, target, cif_id
